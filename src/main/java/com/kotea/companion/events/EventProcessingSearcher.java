...
public class EventProcessingSearcher extends CachingSearcher<KtClassOrObject> {

    private static final Logger LOG = Logger.getInstance(EventProcessingSearcher.class);
    private static final EventProcessingSearcher INSTANCE = new EventProcessingSearcher();

    private EventProcessingSearcher() {
    }
    ...
    protected List<PsiElement> search(@NotNull KtClassOrObject target, @NotNull GlobalSearchScope scope) {
        ...
        for (var ref : ReferencesSearch.search(target, scope, false).findAll()) {
            ...
            if (RoleResolver.roleOf(el, KoTEAElementKind.EVENT) != RoleResolver.Role.PROCESSING) continue;
            results.add(el);
        }
